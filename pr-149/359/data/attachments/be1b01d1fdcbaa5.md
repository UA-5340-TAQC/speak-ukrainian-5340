...
```ts
  ...
  179 |     }): Promise<void> => {
  180 |       test.fail(true, 'Backend bug: API returns 200 OK when approving non-existent registrations');
  181 |       const response: APIResponse = await clubRegistrationClient.approveRegistration(id);
> 182 |       expect(response.ok()).toBeFalsy();
      |                             ^ Error: expect(received).toBeFalsy()
  183 |     });
  184 |   }
  185 | });
