...
Call log:
  - waiting for locator('div.ant-menu-submenu-popup').filter({ visible: true }).locator('li.ant-menu-item-only-child span').filter({ hasText: /^Клуб української мови "Розмовляй"$/ })
